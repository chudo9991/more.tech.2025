...
<style scoped>
/* Removed problematic scrollbar hiding */
...
/* Specifically target BaseButton components */
...
:deep(.btn:focus), :deep(button:focus) {
  border: none !important;
  outline: none !important;
  box-shadow: none !important;
}

/* Removed overly broad global styles that can cause performance issues */

/* Restore necessary borders only where needed */
:deep(.hero-image-placeholder) {
  border: 3px solid rgba(0, 255, 255, 0.4) !important;
}

/* Removed global body/html styles that can cause conflicts */
...
.home::before {
  ...
  height: 100%;
  background: 
    radial-gradient(circle at 20% 80%, rgba(0, 255, 255, 0.08) 0%, transparent 70%),
    radial-gradient(circle at 80% 20%, rgba(138, 43, 226, 0.08) 0%, transparent 70%),
    radial-gradient(circle at 40% 40%, rgba(75, 0, 130, 0.05) 0%, transparent 60%),
    radial-gradient(circle at 90% 90%, rgba(255, 20, 147, 0.05) 0%, transparent 50%),
    radial-gradient(circle at 10% 10%, rgba(30, 144, 255, 0.04) 0%, transparent 60%);
  pointer-events: none;
  z-index: -1;
}
...
/* Кнопки теперь используют BaseButton компонент */
