    ...
    <div class="p-6">
      ...
      <div v-if="resume.total_score !== null && resume.total_score !== undefined" class="mb-4">
        <div class="flex items-center justify-between text-sm mb-2">
          <span class="text-neutral-600 dark:text-neutral-400">Соответствие</span>
          <span class="font-medium text-neutral-900 dark:text-white">{{ resume.total_score }}%</span>
        </div>
...
// Extend HTMLElement interface for custom property
...
.resume-card :deep(.bg-neutral-700) {
...
.resume-card :deep(.bg-neutral-800) {
  background: linear-gradient(135deg, rgba(15, 23, 42, 0.95), rgba(30, 41, 59, 0.95)) !important;
}

.resume-card :deep(.border-neutral-200),
.resume-card :deep(.border-neutral-700) {
...
.resume-card :deep(.hover\\:bg-neutral-700:hover) {
  background: rgba(0, 255, 255, 0.1) !important;
}
